...
class Simulation:
    def __init__(self, T, algorithm, parameters, trial):
        ...
        ranked_agents = np.flip(np.argsort(self.profiles)) # lists the agents in descending order according to sensitivity
    ...
    def get_mean_estimate(self, n, a = None): # estimate for mean of arm n after time self.t
        if a == None:
            ...
            if self.min_width: return 1 / np.sum([self.profiles[a] ** 2 * self.cumulative_record[-1, a, n] for a in range(self.A)]) * np.sum([self.profiles[a] * np.sum([self.action_record[tau, a, n] * self.observations[tau, a, n] for tau in range(self.t)]) for a in range(self.A)])
        ...
        if a != None:
            ...
            return 1 / (self.profiles[a] * self.cumulative_record[-1, a, n]) * np.sum([self.action_record[tau, a, n] * self.observations[tau, a, n] for tau in range(self.t)])
    ...
    def get_epsilon(self, n, a = None):
        if a == None:
            ...
            if self.min_width:
                ...
                return np.sqrt(np.log(2 * self.N * factor / self.delta) / (2 * np.sum([self.profiles[a] ** 2 * self.cumulative_record[-1, a, n] for a in range(self.A)])))
        ...
        if a != None:
            ...
            return 1 / self.profiles[a] * np.sqrt(np.log(2 * self.A * self.N * self.t / self.delta) / (2 * self.cumulative_record[-1, a, n]))
